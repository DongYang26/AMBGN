...
class GCN(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        x11 = F.dropout(x1, self.dropout, training = self.training)
        x2 = self.gc2(x11, adj)
        return x1, x2
...
class SFGCN(nn.Module):
    ...
    def forward(self, x, sadj, fadj):
        hidden_outpute1, emb1 = self.SGCN1(x, sadj)
        hidden_outputc1, com1 = self.CGCN(x, sadj)
        hidden_outputc2, com2 = self.CGCN(x, fadj)
        hidden_outpute2, emb2 = self.SGCN2(x, fadj)
        Xcom = (com1 + com2) / 2
        # attention
        emb = torch.stack([emb1, emb2, Xcom], dim=1)
        emb, att = self.attention(emb)
        output = self.MLP(emb)
        return output, att, emb1, com1, com2, emb2, emb, hidden_outpute2